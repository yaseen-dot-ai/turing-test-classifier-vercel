...
from predict import batch_predict, CANDIDATES, CLASSES
...
def run_experiment(req: RunRequest):
    ...
    for m, y_hat in preds.items():
        ...
        metrics[m] = {"precision": precision, "recall": recall}

    def meets_bar(p):
        return all(v >= .95 for v in p.values())
    eligible = [m for m in models if meets_bar(metrics[m]["precision"])]

    if eligible:
        winner = max(eligible, key=lambda m: sum(metrics[m]["recall"].values()))
    else:
        winner = max(models, key=lambda m: metrics[m]["precision"]["HUMAN"])

    winner_cfg = {"model_type": winner, "version": run_id}

    root = f"runs/{run_id}/"
    s3_put(root + "winner.json", json.dumps(winner_cfg))
    print(json.dumps(metrics))
    s3_put(root + "metrics.json", json.dumps(metrics))
    full_out = df.assign(**{f"{m}_pred": p for m, p in preds.items()})
    ...
    body = {
        "run_id": run_id,
        "winner": winner,
        "precision": metrics[winner]["precision"],
        "recall": metrics[winner]["recall"]
    }
    if req.return_preds:
